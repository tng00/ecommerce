...
# def validate_user_access(get_user):
#     if not (get_user.get("is_customer") or get_user.get("is_admin")):
#         raise HTTPException(
#             status_code=status.HTTP_403_FORBIDDEN,
#             detail="You are not authorized to use this method",
#         )
#     return get_user.get("id")

# async def create_order_in_db(db, user_id, create_order, current_timestamp):
#     query = text(
#         """
#         INSERT INTO orders (user_id, payment_id, order_date, address, status, total_amount)
#         VALUES (:user_id, NULL, :order_date, :address, 'processing', :total)
#         RETURNING id
#         """
#     )
#     result = await db.execute(
...
# async def add_order_items(db, order_id, items):
#     for item in items:
#         query = text(
#             """
#             INSERT INTO order_items (order_id, product_id, quantity, price)
...
# def generate_check(create_order, check_id):
#     check_file_path = os.path.join(CHECKS_DIR, f"{check_id}.pdf")
...
# async def update_order_with_check_id(db, order_id, check_id):
#     query = text(
#         """
#         UPDATE orders
...
#         """
#     )
#     await db.execute(query, {"check_id": check_id, "order_id": order_id})
