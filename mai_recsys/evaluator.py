...
class ModelEvaluator:
    def __init__(self, items, it_full, it_train, it_test) -> None:
        self.items = items
    ...
    def get_not_interacted_items_sample(self, person_id, sample_size, seed=42):
        interacted_items = get_items_interacted(person_id, self.it_full)
    ...
    def evaluate_model_for_user(self, model, person_id):
        ...
        interacted_values_testset = self.it_test.loc[person_id]
    ...
    def evaluate_model(self, model):
        ...
        people_metrics = []
        for idx, person_id in enumerate(list(self.it_test.index.unique().values)):
...
def evaluate(model_evaluator, model):
